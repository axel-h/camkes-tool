...
set(CAMKES_OBJECT_PICKLE "${CMAKE_CURRENT_BINARY_DIR}/object.pickle")
set(CAMKES_OBJECT_FINAL_PICKLE "${CMAKE_CURRENT_BINARY_DIR}/object-final.pickle")
        ...
        /*- for id, e in enumerate(c.from_ends) -*/
            ...
            /*- if e.instance.name == i.name -*/
                ...
                if (NOT "${c_from_template_header}" STREQUAL "")
                    if(NOT EXISTS "${c_from_template_header}")
                        message("${connector_target}: missing FROM_HEADER ${c_from_template_header}")
                    else()
            ...
            /*- if e.instance.name == i.name -*/
                ...
                /*- else -*/
                    ...
                    if (NOT "${c_to_template_header}" STREQUAL "")
                        if(NOT EXISTS "${c_to_template_header}")
                            message("${connector_target}: missing TO_HEADER ${c_to_template_header}")
                        else()
...
CAmkESOutputGenCommand(save-object-state "${CAMKES_OBJECT_PICKLE}")
...
if(CAmkESCapDLStaticAlloc)
    ...
    add_custom_command(
        OUTPUT "${CAPDL_UNTYPED_YAML_FILE}"
        COMMAND
            ${CAPDL_UNTYPED_GEN}
                "--input=${platform_yaml}"
                "--output=${CAPDL_UNTYPED_YAML_FILE}"
    ...
    add_custom_target(untyped_capdl_target DEPENDS "${CAPDL_UNTYPED_YAML_FILE}")
else()
...
add_custom_command(
    OUTPUT ${CAMKES_CDL_TARGET} "${CAMKES_OBJECT_FINAL_PICKLE}"
    COMMAND
        ${CAPDL_LINKER}
            "--object-sizes=$<TARGET_PROPERTY:object_sizes,FILE_PATH>"
            --architecture ${KernelSel4Arch}
            gen_cdl
            "--manifest-in=${CAMKES_OBJECT_PICKLE}"
            "--save-object-state=${CAMKES_OBJECT_FINAL_PICKLE}"
...
add_custom_target(camkes_capdl_target DEPENDS "${CAMKES_CDL_TARGET}" "${CAMKES_OBJECT_FINAL_PICKLE}")
...
if (CAmkESCapDLVerification)
    ...
    CAmkESOutputGenCommand(load-object-state "${CAMKES_OBJECT_FINAL_PICKLE}")
endif()
